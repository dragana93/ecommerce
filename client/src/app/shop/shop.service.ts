...
export class ShopService {
  ...
  getProducts(useCache = true): Observable<Pagination<Product[]>> {
    if(!useCache) this.productCache = new Map<string, Pagination<Product[]>>();
    ...
    return this.http.get<Pagination<Product[]>>(this.baseUrl + 'products', { params }).pipe(map( response => {
  ...
  getBrands(){
    ...
    return this.http.get<Brand[]>(this.baseUrl + 'products/brands').pipe(map( brands => {
      this.brands = brands;
      return brands;
    }));;
  }
  ...
  getTypes(){
    ...
    return this.http.get<Type[]>(this.baseUrl + 'products/types').pipe(map( types => {
      this.types = types;
      return types;
    }));
  }
